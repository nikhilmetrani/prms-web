...
/**
 * @author Rushabh Shah(Security vulnerability fixed. URLs(Web-pages) cannot be accessed, without logging into the system.)
 * @author projects 
 */
...
public class NocturneApplicationController extends HttpServlet {
    ...
    protected void process(HttpServletRequest req, HttpServletResponse resp)
        ...
        if ((null == path) || (path.trim().length() <= 0)) {
            result = "/nocturne.jsp";
        } else if (user == null && (!path.contains("login"))) {
            result = "/pages/home.jsp";
        } else {
